...
import { sandboxPreset, categoryBackgrounds } from '@/config/backgroundPresets'
import { FormulaCarousel } from '@/components/ui/FormulaCarousel'
import { AdFailPopup } from '@/components/ui/AdFailPopup'
...
export function SandboxScreen({
    ...
    const [adFailPopup, setAdFailPopup] = useState<string | null>(null)
    const [sessionUnlockedFormulas, setSessionUnlockedFormulas] = useState<Set<string>>(new Set())
    const canvasRef = useRef<PixiCanvasHandle>(null)

    // 현재 선택된 공식이 잠겨있는지 확인 (광고 제거, 전체 해금 구매, 세션 임시 해금 포함)
    const isCurrentFormulaLocked = !isAdFree && !isAllFormulasUnlocked && !isUnlocked(formulaId) && !sessionUnlockedFormulas.has(formulaId)

    // Helper to render localized challenge mission text
    const renderMission = useCallback(
        (challenge: typeof currentChallenge): string => {
            if (!challenge) return ''

            // Resolve params: LocalizedText objects become localized strings, condition keys become localized conditions
            const resolvedParams: Record<string, string | number> = {}
            for (const [key, value] of Object.entries(challenge.missionParams)) {
                if (typeof value === 'object' && value !== null && 'ko' in value) {
                    // It's a LocalizedText object - localize it
                    resolvedParams[key] = localizeText(value, i18n.language)
                } else if (
                    typeof value === 'string' &&
                    (key === 'condition' || key === 'cond1' || key === 'cond2')
                ) {
                    // It's a condition key - get localized condition text
                    resolvedParams[key] = t(`sandbox.conditions.${value}`)
                } else {
                    resolvedParams[key] = value as string | number
                }
            }

            return t(challenge.missionKey, resolvedParams)
        },
        [i18n.language, t]
    )
    ...
    const getChallengeHint = useCallback((): string | null => {
        ...
        const params = currentChallenge.missionParams

        if (currentChallenge.type === 'target') {
            // Use value from missionParams directly
            ...
            if (!isNaN(target)) {
                ...
                if (Math.abs(diff) <= tolerance) {
                    return t('sandbox.hints.almostThere')
                } else if (diff > rangeSize * 0.3) {
                    return t('sandbox.hints.goHigher')
                } else if (diff > 0) {
                    return t('sandbox.hints.aBitMore')
                } else if (diff < -rangeSize * 0.3) {
                    return t('sandbox.hints.goLower')
                } else {
                    return t('sandbox.hints.aBitLess')
                }
            }
        } else if (currentChallenge.type === 'range') {
            // Use start/end from missionParams directly
            const rangeStart = parseFloat(String(params.start))
            const rangeEnd = parseFloat(String(params.end))

            if (!isNaN(rangeStart) && !isNaN(rangeEnd)) {
                if (currentValue < rangeStart) {
                    return t('sandbox.hints.intoRangeUp')
                } else if (currentValue > rangeEnd) {
                    return t('sandbox.hints.intoRangeDown')
                } else {
                    return t('sandbox.hints.inRange')
                }
            }
        } else if (currentChallenge.type === 'condition') {
            // Check condition key from missionParams
            const conditionKey = params.condition || params.cond1
            if (conditionKey === 'maximum') {
                if (currentValue < max * 0.9) {
                    return t('sandbox.hints.maximize')
                }
            } else if (conditionKey === 'minimum') {
                if (currentValue > min + rangeSize * 0.1) {
                    return t('sandbox.hints.minimize')
                }
            } else if (conditionKey === 'middle') {
                const mid = (min + max) / 2
                if (currentValue < mid - rangeSize * 0.1) {
                    return t('sandbox.hints.toMiddleUp')
                } else if (currentValue > mid + rangeSize * 0.1) {
                    return t('sandbox.hints.toMiddleDown')
                }
            }
        }

        return null
    }, [currentChallenge, variables, formula, t])
    ...
    const handleSubmitChallenge = useCallback(() => {
        ...
        if (currentChallenge.condition(variables)) {
            ...
            const hint = getChallengeHint() || t('sandbox.hints.tryAgain')
    ...
    const handleCarouselSlideChange = useCallback((category: FormulaCategory, _index: number) => {
    ...
    const handleUnlockFormula = useCallback(
        (formulaIdToUnlock: string) => {
            showRewardAd(
                ...
                () => {
                    // 광고 로드 실패 - 팝업으로 안내
                    console.log('Reward ad failed, showing ad fail popup')
                    setAdFailPopup(formulaIdToUnlock)
                }
            )
        },
    ...
    // 광고 실패 팝업 확인 - 세션 한정 임시 열람 (영구 해금 아님)
    const handleAdFailConfirm = useCallback(() => {
        if (adFailPopup) {
            setSessionUnlockedFormulas(prev => new Set(prev).add(adFailPopup))
            setAdFailPopup(null)
        }
    }, [adFailPopup])
    ...
    return (
        ...
        >
                            ...
                            >
                                {condition.type === 'prerequisite' && prerequisiteFormula ? (
                                    <div className="text-center px-4">
                                        ...
                                        >
                                            ...
                                            <span className="text-white/80 text-sm font-bold">
                                                {t('sandbox.locked.prerequisite')}
                                            </span>
                                        </div>
                                        <p className="text-white/60 text-xs leading-relaxed">
                                            {t('sandbox.locked.completeChallenge', {
                                                formula: localizeText(
                                                    prerequisiteFormula.name,
                                                    i18n.language
                                                ),
                                            })}
                                        </p>
                                    ...
                                    >
                                        ...
                                        {isRewardAdLoading
                                            ? t('sandbox.locked.loading')
                                            : t('sandbox.locked.unlock')}
                                    </button>
                                )}
                            </div>
                        )
                ...
                >
                    ...
                    >
                        ...
                        <span className="text-xs text-amber-200 font-medium flex-1">
                            {renderMission(currentChallenge)}
                        </span>
                        {/* Score & Combo */}
                            ...
                            >
                                <div className="grid grid-cols-2 gap-3">
                                    {filteredFormulas.map((f) => {
                                        ...
                                        const isLocked = !isAdFree && !isAllFormulasUnlocked && !isUnlocked(f.id) && !sessionUnlockedFormulas.has(f.id)
                                        return (
            ...
            {/* Ad Fail Popup */}
            {adFailPopup && <AdFailPopup onConfirm={handleAdFailConfirm} />}

            {/* Formula Select Overlay - Carousel View (same as HomeScreen) */}
            {welcomePhase === 'select' && (
                <div
                ...
                >
                    ...
                    >
                        ...
                        <div className="flex-1 flex flex-col justify-center overflow-hidden">
                            <FormulaCarousel
                                onSelectFormula={handleSelectFromWelcome}
