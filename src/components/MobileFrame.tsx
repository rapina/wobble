 ...
 * Can be toggled off via DevOptionsModal for PC mode.
 */
export function MobileFrame({ children }: MobileFrameProps) {
    const [showFrame, setShowFrame] = useState(false)
    const { isFrameHidden } = useMobileFrameStore()
    ...
    // On native or when frame is hidden (PC mode), render children directly
    if (!showFrame || isFrameHidden) {
        return <>{children}</>
    }
