...
// AbyssTentacle removed - wormhole now drifts autonomously
import {
    StageConfig,
...
export class WobblediverScene extends BaseMiniGameScene {
    ...
    // Wormhole drift state (replaces abyss tentacles)
    private wormholeDriftTime = 0
    private wormholeDriftSpeedX = 1.0
    private wormholeDriftSpeedY = 0.7
    private wormholeDriftAmplitudeX = 0
    private wormholeDriftAmplitudeY = 0
    ...
    // Deferred filter application to avoid PixiJS v8 Batcher initialization race
    private _filterDeferFrames = 0
    ...
    private setupBackground(): void {
        ...
        // Filter applied in applyDeferredFilters() to avoid PixiJS v8 Batcher race
    ...
    private updatePerkIconsDisplay(): void {
        ...
        // Clear all old children (icons, backgrounds, stack badges)
        while (this.perkIconsContainer.children.length > 0) {
            const child = this.perkIconsContainer.children[0]
            this.perkIconsContainer.removeChild(child)
            child.destroy()
        }
        this.perkIconTexts = []
    ...
    private getDifficultyHints(_stageNumber: number): string[] {
        ...
        if (factors.abyssTentacleCount > 0) {
            hints.push(i18n.t('wobblediver.hints.wormholeDrifts'))
        }
    ...
    private onResultFadeOutComplete(): void {
        ...
        if (this.runMode && this.runNodeId) {
            ...
            // Endless mode: show perk selection every 3 stages
            // Stage 3, 6, 9, 12, etc. (roundNumber is 1-indexed at this point for completed stage)
            if (this.roundNumber % 3 === 0) {
                console.log('[Wobblediver] Calling showPerkSelection for endless mode (stage', this.roundNumber, ')')
                this.showPerkSelection()
            } else {
                console.log('[Wobblediver] Skipping perk selection (stage', this.roundNumber, ')')
                this.startNewRound()
            }
        }
    }
    ...
    private startNewRound(): void {
        ...
        // Setup wormhole drift based on stage config
        this.setupWormholeDrift(config)
    ...
    private positionGoalFromConfig(config: StageConfig): void {
        ...
        this.wormhole.resetCollapse()
        this.wormhole.container.visible = true
    ...
    /**
     * Setup wormhole drift parameters based on stage config
     * Wormhole drifts in a Lissajous-curve pattern around its base position
     */
    private setupWormholeDrift(config: StageConfig): void {
        // Reset drift time for each new stage
        this.wormholeDriftTime = Math.random() * Math.PI * 2 // Random phase

        // Drift intensity scales with water level and tentacle count
        const tentacleInfluence = Math.min(1, config.abyssTentacles.length * 0.35)
        const waterInfluence = config.waterLevelRise

        // Combine influences: more tentacles configured = more drift
        const driftIntensity = Math.max(tentacleInfluence, waterInfluence)

        // Amplitude: 0 when no drift factors, up to 80px horizontal and 40px vertical
        this.wormholeDriftAmplitudeX = driftIntensity * 80
        this.wormholeDriftAmplitudeY = driftIntensity * 40

        // Speed: gentle at low intensity, faster at high
        this.wormholeDriftSpeedX = 0.5 + driftIntensity * 0.8
        this.wormholeDriftSpeedY = 0.35 + driftIntensity * 0.6
    }

    /**
     * Update wormhole drift animation
     * Smooth Lissajous-curve movement around the base position
     */
    private updateWormholeDrift(deltaSeconds: number): void {
        if (!this.currentStageConfig) return
        if (this.wormholeDriftAmplitudeX <= 0 && this.wormholeDriftAmplitudeY <= 0) return

        this.wormholeDriftTime += deltaSeconds

        // Base wormhole position from config
        const baseX = this.currentStageConfig.wormhole.x + this.gameOffsetX
        const baseY = this.currentStageConfig.wormhole.y

        // Lissajous curve with irrational frequency ratio for non-repeating motion
        const t = this.wormholeDriftTime
        const dx = Math.sin(t * this.wormholeDriftSpeedX * 1.0) * this.wormholeDriftAmplitudeX
            + Math.sin(t * this.wormholeDriftSpeedX * 1.7) * this.wormholeDriftAmplitudeX * 0.3
        const dy = Math.sin(t * this.wormholeDriftSpeedY * 1.3) * this.wormholeDriftAmplitudeY
            + Math.cos(t * this.wormholeDriftSpeedY * 2.1) * this.wormholeDriftAmplitudeY * 0.2
    ...
    private restartCurrentStage(): void {
        ...
        // Reset wormhole to original position and state
        this.wormhole.resetCollapse()
        this.wormhole.container.visible = true
    ...
    private positionGoal(): void {
        ...
        // Wormhole movement is controlled by drift system
    ...
    protected updateGame(deltaSeconds: number): void {
        ...
        // Update wormhole drift movement
        this.updateWormholeDrift(effectiveDelta)
        ...
        if (this.wobble) {
            ...
            // Handle pending result (character scales out, wormhole collapses)
            if (this.pendingResult && this.pendingResultTimer > 0) {
                this.pendingResultTimer -= effectiveDelta

                // Animate wobble scaling out (getting bigger and fading)
                if (this.wobble) {
                    const totalDuration = this.SUCTION_ANIMATION_DURATION
                    const elapsed = totalDuration - this.pendingResultTimer
                    const progress = Math.min(1, elapsed / totalDuration)

                    // Ease-out curve for smooth scale-out
                    const easeOut = 1 - Math.pow(1 - progress, 3)

                    // Scale UP (character grows and fades away)
                    const scale = 1 + easeOut * 1.5
                    this.wobble.container.scale.set(scale)

                    // Fade out
                    this.wobble.container.alpha = Math.max(0, 1 - easeOut)
                }

                // Wormhole collapse is handled by Wormhole.update() via startCollapse()
                ...
                if (this.pendingResultTimer <= 0) {
                    ...
                    // Reset wormhole collapse for next round
                    this.wormhole.resetCollapse()
                    this.wormhole.container.visible = true
                }
            }
    ...
    private checkCollisions(): void {
        ...
        if (wormholeHit.hit && !this.pendingResult) {
            ...
            this.wormhole.startCollapse(this.SUCTION_ANIMATION_DURATION)
    ...
    /**
     * Apply filters after a few render frames to avoid PixiJS v8 Batcher init race.
     * The Batcher's internal batch set may be null on the very first render frame,
     * causing "Cannot read properties of null (reading 'clear')" in FilterPipe.pop.
     */
    private applyDeferredFilters(): void {
        if (this._filtersApplied) return
        this._filterDeferFrames++
        if (this._filterDeferFrames < 3) return
        this._filtersApplied = true
        try {
            if (this.abyssBackgroundSprite && this.abyssBackgroundFilter) {
                this.abyssBackgroundSprite.filters = [this.abyssBackgroundFilter]
            }
            if (this.abyssFluidSprite && this.abyssFluidFilter) {
                this.abyssFluidSprite.filters = [this.abyssFluidFilter]
            }
        } catch (e) {
            console.warn('[WobblediverScene] Error applying deferred filters:', e)
        }
    }
    ...
    protected animate(ticker: Ticker): void {
        ...
        // Defer filter application to avoid PixiJS v8 Batcher race condition
        this.applyDeferredFilters()
    ...
    protected onDifficultyChange(config: DifficultyConfig, previousPhase: DifficultyPhase): void {
        // Note: goalWidthScale is now controlled by StageGenerator, wormhole movement by drift system
        switch (config.phase) {
            case 'easy':
    ...
    protected onGameReset(): void {
        ...
        // Clear and hide perk icons display
        if (this.perkIconsContainer) {
            // Remove all children to prevent stale icons showing
            while (this.perkIconsContainer.children.length > 0) {
                const child = this.perkIconsContainer.children[0]
                this.perkIconsContainer.removeChild(child)
                child.destroy()
    ...
    public destroy(): void {
        ...
        try {
            ...
            console.warn('[WobblediverScene] Background filter cleanup error:', e)
        }
        try {
            if (this.abyssFluidSprite) {
            ...
            console.warn('[WobblediverScene] Fluid filter cleanup error:', e)
        }
