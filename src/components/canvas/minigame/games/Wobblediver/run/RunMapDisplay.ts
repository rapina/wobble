...
export class RunMapDisplay {
    ...
    private setupBackground(): void {
        ...
        // Filter applied in update() after a few frames to avoid PixiJS v8 Batcher race
    }
    ...
    update(deltaSeconds: number): void {
        ...
        if (!this._filtersApplied && this.backgroundSprite && this.backgroundFilter) {
            this._filterDeferFrames++
            if (this._filterDeferFrames >= 3) {
                this._filtersApplied = true
                try {
                    this.backgroundSprite.filters = [this.backgroundFilter]
                } catch (e) {
                    console.warn('[RunMapDisplay] Error applying deferred filter:', e)
                }
            }
        }
    ...
    destroy(): void {
        ...
        // Clear filters from sprite BEFORE destroying the filter to avoid Batcher race
        try {
            if (this.backgroundSprite) {
            ...
            console.warn('[RunMapDisplay] Filter cleanup error:', e)
        }
