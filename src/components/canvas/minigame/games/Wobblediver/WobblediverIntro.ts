...
class MiniJellyfishDisplay extends Container {
    ...
    private setup(): void {
        ...
        this.contentContainer.mask = maskGraphics
        ...
        for (let i = 0; i < 8; i++) {
            ...
            this.contentContainer!.addChild(particle)
        }

        this.contentContainer!.addChild(this.graphics)
    ...
    destroy(): void {
        // Clear mask reference before destroying to avoid PixiJS render errors
...
export class WobblediverIntro {
    ...
    private createUI(): void {
        // Clean up mask references before removing children to avoid PixiJS render errors
        if (this.miniJellyfish) {
            this.miniJellyfish.destroy()
        }
        if (this.miniWormhole) {
            this.miniWormhole.destroy()
        }
        if (this.miniAbyss) {
            this.miniAbyss.destroy()
        }
        this.container.removeChildren()
