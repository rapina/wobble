...
/**
 * Blob shape types - 각 shape는 고유한 역할/성격을 가짐
 *
 * - 'circle': 기본 원형 블롭 (주인공/공격자 역할)
 *   둥글고 친근한 느낌, 주로 액션을 주도함
 *
 * - 'square': 사각형 블롭 (피해자/victim 역할)
 *   당하는 역할, 고통받는 표정과 잘 어울림
 *
 * - 'triangle': 삼각형 블롭 (날카로운/공격적 역할)
 *   뾰족하고 위협적인 느낌, 적이나 장애물 역할
 *
 * - 'star': 별 모양 블롭 (특별한/보너스 역할)
 *   빛나는 존재, 보상이나 파워업 역할
 *
 * - 'diamond': 다이아몬드 블롭 (귀중한/목표 역할)
 *   가치 있는 존재, 수집 대상이나 목표물 역할
 *
 * - 'pentagon': 오각형 블롭 (안정적/방어적 역할)
 *   균형 잡힌 느낌, 방어나 지지 역할
 */
export type BlobShape = 'circle' | 'square' | 'triangle' | 'star' | 'diamond' | 'pentagon';
...
const defaultOptions: Required<BlobOptions> = {
    size: 60,
...
export class Blob extends Container {
    ...
    private options: Required<BlobOptions>;
    ...
    private drawBody(): void {
        ...
        switch (shape) {
            case 'square':
                this.drawSquareBody(g, size, scaleX, scaleY, wobblePhase);
                break;
            case 'triangle':
                this.drawTriangleBody(g, size, scaleX, scaleY, wobblePhase);
                break;
            case 'star':
                this.drawStarBody(g, size, scaleX, scaleY, wobblePhase);
                break;
            case 'diamond':
                this.drawDiamondBody(g, size, scaleX, scaleY, wobblePhase);
                break;
            case 'pentagon':
                this.drawPentagonBody(g, size, scaleX, scaleY, wobblePhase);
                break;
            default:
                // 원형 블롭 (기본)
    ...
    private drawSquareBody(g: Graphics, size: number, scaleX: number, scaleY: number, wobblePhase: number): void {
        ...
        const cornerRadius = size * 0.15; // 둥근 모서리
        const wobbleAmount = size * 0.03;

        // Subtle wobble for square (less wobbly than circle)
        ...
        // Draw rounded rectangle with wobble
    ...
    private drawLocoRocoEyes(
        ...
        if (expression === 'struggle' || expression === 'dizzy') {
            ...
            // Normal/happy/excited eyes - simple dot style
            [-1, 1].forEach((side) => {
                const cx = side * eyeSpacing + lookX * 0.5;
                // Main black eye
            ...
            // Extra small sparkle for excited only
    ...
    private drawLocoRocoMouth(
        ...
        if (expression === 'surprised') {
            // Big O mouth - LocoRoco signature
            ...
            // Happy smile - LocoRoco signature curve
    ...
    /**
     * Update blob options and redraw
     */
    public updateOptions(options: Partial<BlobOptions>): void {
     ...
     * Get current options
     */
    public getOptions(): Required<BlobOptions> {
        return { ...this.options };
    }
}
