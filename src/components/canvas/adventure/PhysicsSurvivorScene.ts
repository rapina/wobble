...
export class PhysicsSurvivorScene extends AdventureScene {
    ...
    private drawUIHexagon(g: Graphics, x: number, y: number, size: number, fill?: number, stroke?: number, strokeWidth = 2): void {
        const points: number[] = [];
        for (let i = 0; i < 6; i++) {
    ...
    private createResultUI(): void {
        ...
        if (this.activePerks.length > 0) {
            const hexSize = 18;
            const iconGap = 8;
            const totalWidth = this.activePerks.length * (hexSize * 2) + (this.activePerks.length - 1) * iconGap;
    ...
    private createResultButton(icon: string, offsetX: number, color: number, onClick: () => void): Container {
    ...
    private drawHexagon(g: Graphics, x: number, y: number, size: number, fill?: number, stroke?: number, strokeWidth = 3): void {
        const points: number[] = [];
        for (let i = 0; i < 6; i++) {
            const angle = (Math.PI / 3) * i - Math.PI / 2;
            points.push(x + size * Math.cos(angle));
    ...
    private createCharacterSelectUI(): void {
        ...
        PLAYABLE_CHARACTERS.forEach((shape, index) => {
            ...
            this.drawHexagon(outerHex, 0, 0, hexSize + 6, 0x0a0a10, isSelected ? character.color : 0x3a3040, 3);
            charContainer.addChild(outerHex);

            // Inner hexagon background
            const innerHex = new Graphics();
            this.drawHexagon(innerHex, 0, 0, hexSize, isSelected ? 0x2a2530 : 0x15131a, character.color, 2);
        ...
        this.characterDescText = new Text({ text: '', style: new TextStyle({ fontSize: 1, fill: 0x000000 }) });
    ...
    private updateStatsDisplay(stats: WobbleStats, color: number): void {
        ...
        statItems.forEach((item, index) => {
            ...
            barFill.roundRect(x - barWidth / 2 + 1, barY - barHeight / 2 + 1, fillWidth - 2, barHeight - 2, 1);
