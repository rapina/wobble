...
export class IntroScene {
    ...
    update(deltaSeconds: number): void {
        ...
        if (this.wobble) {
            ...
            const wobbleY = step.embedType !== 'none' ? this.centerY - 200 : this.centerY - 140
    ...
    private createUI(): void {
        // Get references to children before removing
        const oldChildren = [...this.sceneContainer.children]
        this.sceneContainer.removeChildren()

        // Defer destruction to next frame to avoid TexturePool conflicts during rendering
        ...
        // Shifted up by ~80px for better vertical balance
        const wobbleSize = hasEmbed ? 50 : 70
        const wobbleY = hasEmbed ? this.centerY - 200 : this.centerY - 140
        ...
        const cardY = hasEmbed ? this.centerY - 120 : this.centerY - 30
