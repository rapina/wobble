...
export class TorqueScene extends BaseScene {
    ...
    private drawForce(pentaX: number, pentaY: number): void {
        ...
        g.lineTo(forceStartX + Math.cos(arrowAngle - 2.7) * arrowSize, forceStartY + Math.sin(arrowAngle - 2.7) * arrowSize);
        g.moveTo(forceStartX, forceStartY);
        g.lineTo(forceStartX + Math.cos(arrowAngle + 2.7) * arrowSize, forceStartY + Math.sin(arrowAngle + 2.7) * arrowSize);
    ...
    private drawUI(torque: number): void {
        ...
        g.roundRect(torqueBarX + 2, torqueBarY + torqueBarHeight - 2 - fillHeight, 16, fillHeight, 2);

        const torqueColor = Math.abs(torque) > 100 ? 0xff6b6b : Math.abs(torque) > 50 ? 0xf7dc6f : 0x4ecdc4;
        g.fill(torqueColor);
