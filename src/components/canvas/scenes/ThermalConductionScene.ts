...
export class ThermalConductionScene extends BaseScene {
    ...
    private drawUI(Q: number, k: number, A: number, dT: number, L: number, barStartX: number, barEndX: number, barHeight: number): void {
        ...
        this.drawTempIndicator(g, barEndX + 35, this.centerY - 50, Math.round(this.coldTemp), 0x3498db);
    ...
    private drawTempIndicator(g: Graphics, x: number, y: number, temp: number, color: number): void {
        // Temperature bubble
        g.circle(x, y, 16);
