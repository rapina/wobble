...
export class LorentzScene extends BaseScene {
    ...
    private drawMagneticField(B: number): void {
        ...
        for (let x = 30; x < this.width - 20; x += spacing) {
            for (let y = 30; y < this.height - 20; y += spacing) {
                ...
                const nearCharge = Math.sqrt(Math.pow(x - this.posX, 2) + Math.pow(y - this.posY, 2)) < 50;

                // Outer ring (pulsing)
                const ringSize = nearCharge ? 8 * pulse : 6;
                g.circle(x, y, ringSize);
                g.stroke({ color: 0x4a9eff, width: 1.5, alpha: fieldAlpha * (nearCharge ? 1 : 0.6) });
    ...
    private drawTrail(): void {
        ...
        for (let i = 0; i < segments; i += 2) {
            ...
            g.moveTo(
                this.centerX + Math.cos(a1) * radius,
                this.centerY + Math.sin(a1) * radius
            );
            g.lineTo(
                this.centerX + Math.cos(a2) * radius,
                this.centerY + Math.sin(a2) * radius
            );
        }
    ...
    private drawVectors(velocityDir: number, v: number, B: number, F: number, radius: number): void {
    ...
    private drawArrow(g: Graphics, x: number, y: number, angle: number, length: number, color: number, label: string): void {
