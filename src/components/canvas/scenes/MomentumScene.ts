    ...
    private updatePhysics(delta: number): void {
        ...
        switch (this.phase) {
            ...
            case 'reset':
                ...
                if (
                    Math.abs(this.blob1X - 80) < 5 &&
                    Math.abs(this.blob2X - (this.width - 120)) < 5
                ) {
                    this.blob1X = 80
                    this.blob2X = this.width - 120
    ...
    private drawImpact(delta: number): void {
        ...
        if (this.impactTimer > 0) {
            ...
            for (let i = 0; i < 8; i++) {
                ...
                g.moveTo(impactX + Math.cos(angle) * innerR, impactY + Math.sin(angle) * innerR)
                g.lineTo(impactX + Math.cos(angle) * outerR, impactY + Math.sin(angle) * outerR)
            }
