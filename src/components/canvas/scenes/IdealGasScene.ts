...
export class IdealGasScene extends BaseScene {
    ...
    protected animate(ticker: Ticker): void {
        ...
        this.pressure = (this.wallHits * this.gasBlobs.length * this.temperature) / (currentHeight * 10);
    ...
    private drawScale(): void {
        ...
        g.rect(this.containerX, this.pistonY, this.containerWidth, this.containerBottomY - this.pistonY);
    ...
    private drawContainer(): void {
        ...
        // Back wall (darker)
        g.rect(this.containerX + 4, effectiveTop + 4, this.containerWidth - 8, this.containerBottomY - effectiveTop - 8);
        ...
        // Left wall
        g.rect(this.containerX - wallThickness, effectiveTop, wallThickness, this.containerBottomY - effectiveTop + wallThickness);
        g.fill(0x555566);

        // Right wall
        g.rect(this.containerX + this.containerWidth, effectiveTop, wallThickness, this.containerBottomY - effectiveTop + wallThickness);
        g.fill(0x555566);

        // Bottom wall
        g.rect(this.containerX - wallThickness, this.containerBottomY, this.containerWidth + wallThickness * 2, wallThickness);
        g.fill(0x555566);

        // Wall highlights
        g.rect(this.containerX - wallThickness, effectiveTop, 2, this.containerBottomY - effectiveTop + wallThickness);
        ...
        // Piston shadow
        g.roundRect(this.containerX - 8, effectiveTop - 2 + pistonPulse, this.containerWidth + 16, pistonHeight + 4, 4);
        g.fill({ color: 0x000000, alpha: 0.3 });

        // Piston body
        g.roundRect(this.containerX - 6, effectiveTop - 4 + pistonPulse, this.containerWidth + 12, pistonHeight, 4);
        g.fill(0x777788);

        // Piston highlight
        g.roundRect(this.containerX - 4, effectiveTop - 2 + pistonPulse, this.containerWidth + 8, 4, 2);
        g.fill({ color: 0x9999aa });

        // Piston handle
        const handleWidth = 40;
        const handleHeight = 30;
        g.roundRect(this.centerX - handleWidth / 2, effectiveTop - handleHeight - 4 + pistonPulse, handleWidth, handleHeight, 6);
        g.fill(0x666677);
        ...
        if (this.volumeChangeFlash > 0.1) {
            ...
            if (expanding) {
                ...
                this.drawArrow(g, this.containerX + this.containerWidth + 25, arrowY, 'down', arrowAlpha);
            } else {
                // Up arrows when compressing
                this.drawArrow(g, this.containerX - 25, arrowY, 'up', arrowAlpha);
                this.drawArrow(g, this.containerX + this.containerWidth + 25, arrowY, 'up', arrowAlpha);
            }
        }
    }

    private drawArrow(g: Graphics, x: number, y: number, direction: 'up' | 'down', alpha: number): void {
        const size = 12;
