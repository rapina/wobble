...
export class TimeDilationScene extends BaseScene {
    ...
    private drawEffects(v: number, orbitRX: number, orbitRY: number, startX: number, startY: number): void {
        ...
        // Stars in background
        const starPositions = [
            { x: 30, y: 40 }, { x: this.width - 40, y: 50 },
            { x: 50, y: this.height - 60 }, { x: this.width - 60, y: this.height - 50 },
            { x: this.width / 2, y: 30 },
        ];
    ...
    private drawDigit(g: Graphics, x: number, y: number, digit: number, color: number): void {
        ...
        if (segs[0]) { g.moveTo(x - w/2, y - h/2); g.lineTo(x + w/2, y - h/2); g.stroke({ color, width: t, alpha: 0.9 }); }
        if (segs[1]) { g.moveTo(x + w/2, y - h/2); g.lineTo(x + w/2, y); g.stroke({ color, width: t, alpha: 0.9 }); }
        if (segs[2]) { g.moveTo(x + w/2, y); g.lineTo(x + w/2, y + h/2); g.stroke({ color, width: t, alpha: 0.9 }); }
        if (segs[3]) { g.moveTo(x - w/2, y + h/2); g.lineTo(x + w/2, y + h/2); g.stroke({ color, width: t, alpha: 0.9 }); }
        if (segs[4]) { g.moveTo(x - w/2, y); g.lineTo(x - w/2, y + h/2); g.stroke({ color, width: t, alpha: 0.9 }); }
        if (segs[5]) { g.moveTo(x - w/2, y - h/2); g.lineTo(x - w/2, y); g.stroke({ color, width: t, alpha: 0.9 }); }
        if (segs[6]) { g.moveTo(x - w/2, y); g.lineTo(x + w/2, y); g.stroke({ color, width: t, alpha: 0.9 }); }
    }
}
