...
export class EntropyScene extends BaseScene {
    ...
    protected animate(ticker: Ticker): void {
        ...
        if (this.dividerOpen > 0.1) {
            this.particleBlobs.forEach((p1, i) => {
                this.particleBlobs.forEach((p2, j) => {
                    ...
                    if (dist < 50) {
                        const transfer = heatTransferRate * (p1.temperature - p2.temperature) * delta;
        ...
        this.particleBlobs.forEach((p, i) => {
            ...
            const expression = p.temperature > 0.7 ? 'hot' : p.temperature > 0.4 ? 'happy' : 'sleepy';
        ...
        this.heatParticles = this.heatParticles.filter((hp) => hp.life > 0 && hp.x < this.centerX + 100);
    ...
    private drawThermometer(T: number): void {
        ...
        g.roundRect(thermoX + 2, thermoY + thermoHeight - fillHeight - 5, thermoWidth - 4, fillHeight, 6);
