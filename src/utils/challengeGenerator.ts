...
import { Challenge, ChallengeType, MissionParamValue } from '../stores/challengeStore'
import { t } from './localization'

// Condition templates for 'condition' type challenges
// The key matches the i18n key at sandbox.conditions.{key}
interface ConditionTemplate {
    key: 'maximum' | 'minimum' | 'middle'
...
function generateTargetChallenge(formula: Formula, variables: Variable[]): Challenge {
    ...
    return {
        ...
        missionKey: 'sandbox.missions.target',
        missionParams: {
            variable: targetVar.name, // LocalizedText object - will be resolved at render time
            value: displayValue,
            unit: targetVar.unit,
        },
...
function generateRangeChallenge(formula: Formula, outputVars: Variable[]): Challenge {
    ...
    return {
        ...
        type: 'range',
        formulaId: formula.id,
        missionKey: 'sandbox.missions.range',
        missionParams: {
...
function generateConditionChallenge(formula: Formula, inputVars: Variable[]): Challenge {
    ...
    // Build mission params based on single or double condition
    const missionKey =
        var2 && cond2 ? 'sandbox.missions.conditionDouble' : 'sandbox.missions.conditionSingle'

    const missionParams: Record<string, MissionParamValue> =
        var2 && cond2
            ? {
                  var1: var1.name,
                  cond1: cond1.key, // Will be resolved via sandbox.conditions.{key}
                  var2: var2.name,
                  cond2: cond2.key,
              }
            : {
