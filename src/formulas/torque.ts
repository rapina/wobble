...
export const torque: Formula = {
    ...
    getInsight: (vars) => {
        const tau = vars['τ']
        if (tau < 5) return { ko: '문고리 살짝 돌리는 정도야', en: 'Barely turning a door handle', ja: 'ドアノブをそっと回す程度' }
        if (tau < 20) return { ko: '병뚜껑 여는 정도야', en: 'Opening a bottle cap', ja: 'ボトルキャップを開ける程度' }
        if (tau < 50) return { ko: '렌치로 볼트 조이는 힘이야', en: 'Tightening a bolt with a wrench', ja: 'レンチでボルトを締める力' }
        if (tau < 100) return { ko: '자전거 페달 밟는 힘이야', en: 'Pedaling a bicycle', ja: '自転車のペダルをこぐ力' }
        if (tau < 200) return { ko: '무거운 문을 여는 힘이야', en: 'Opening a heavy door', ja: '重いドアを開ける力' }
